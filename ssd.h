 ...
 */
enum ftl_implementation {IMPL_PAGE, IMPL_BAST, IMPL_FAST, IMPL_DFTL, IMPL_BIMODAL, IMPL_DATA, IMPL_NEW};
...
class FtlImpl_DATAftl : public FtlParent
{
public:
	FtlImpl_DATAftl(Controller &controller);
	~FtlImpl_DATAftl();
	enum status read(Event &event);
	enum status write(Event &event);
	enum status trim(Event &event);
